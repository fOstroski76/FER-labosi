...
class UMDist(MixtureDist):
    ...
    def random(cls, K: int) -> UMDist:
        a = np.random.uniform(size=K).astype(np.float32)
        b = a + np.random.uniform(size=K).astype(np.float32)  # ensure b > a
        pi = np.random.dirichlet(5 * np.ones(K)).astype(np.float32)  # mixing coefficients
        return cls(pi, a, b)

    def sample(self, L: int) -> NumT:
        choices = np.random.choice(np.arange(self.K), size=L, p=self.pi)  # select components
        sample = np.random.uniform(self.a[choices], self.b[choices]).astype(np.float32)  # sample from chosen component
        return sample
